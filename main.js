...
const filter = () => {
    ...
    allJobs.forEach((offer) => {
        ...
        const allJobsTagSectionChildernsArrays = [
            ...allJobsTagSectionChilderns,
        ];
        const allJobsTagSectionChildernsStrings = [];
...
const generateTag = (tagName) => {
    ...
    jobSectionFilterDelete.addEventListener("click", () => {
        ...
        const test = selectedTags.indexOf(jobSectionFilterTag.innerText);
        if (test == 0) {
            selectedTags.shift();
        }
...
const render = (id, company, logo, newv, featured, position, role, postedAt, contract, location, languages, tools) => {
...
async function printData() {
    ...
    json.forEach(({ id, company, logo, newv, featured, position, role, postedAt, contract, location, languages, tools, }) => {
        render(id, company, logo, newv, featured, position, role, postedAt, contract, location, languages, tools);
    });
}
printData();
clearButton?.addEventListener("click", () => {
    selectedTags.length = 0;
    const filterTagsToDelete = document.querySelectorAll(".jobSection__filter--tag");
